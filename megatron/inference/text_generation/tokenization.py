...
def tokenize_prompts_local(prompts=None, tokens_to_generate=None,
    ...
    print("### before _tokenize_prompts_and_batch")
    prompts_tokens_cuda_long_tensor, prompts_length_cuda_long_tensor = \
        _tokenize_prompts_and_batch1(prompts, tokens_to_generate, add_BOS)
    print("### after _tokenize_prompts_and_batch")
...
def _tokenize_prompts_and_batch1(prompts, tokens_to_generate, add_BOS):
    ...
    eod_token = local_tokenizer.encode('<|im_end|>')[0]
    ...
    if add_BOS:
        prompts_tokens = [[eod_token] + tokenizer.encode(prompt)
    ...
    samples_length = tokens_to_generate
    ...
    prompts_tokens_tensor = torch.tensor(prompts_tokens, dtype=torch.long, device='npu')
    prompts_length_tensor = torch.tensor(prompts_length, dtype=torch.long, device='npu')
