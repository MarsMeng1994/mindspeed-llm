...
from .utils import unwrap_model, print_rank_0, append_to_progress_log, is_last_rank, print_rank_last
...
def get_checkpoint_name_with_diff_p(checkpoints_path, iteration, release=False,
                        pipeline_parallel=None,
                        tensor_rank=None, pipeline_rank=None,
                        expert_parallel=None, expert_rank=None,
                        return_base_dir=False, basename="model_optim_rng.pt", old_pp_num=None, old_ep_num=None):
    """Determine the directory name for this rank's checkpoint."""
    if release:
        directory = 'release'
    else:
    ...
    if not pipeline_parallel:
        ...
        common_path = os.path.join(checkpoints_path, directory,
                f'mp_rank_{tensor_rank:02d}_{int(pipeline_rank // (mpu.get_pipeline_model_parallel_world_size()//old_pp_num)):03d}')

    if expert_parallel:
        if old_ep_num is not None:
            common_path = common_path + f'_{int(expert_rank//(mpu.get_expert_model_parallel_world_size()//old_ep_num)):03d}'
        else:
            common_path = common_path + f'_{expert_rank:03d}'
    # print("### get_checkpoint_name_with_diff_p pp_rank: {}, ep_rank: {}, path: {}\n".format(mpu.get_pipeline_model_parallel_rank(), mpu.get_expert_model_parallel_rank(), os.path.join(common_path, basename)))
    return os.path.join(common_path, basename)

def get_checkpoint_names_with_diff_p(checkpoints_path, iteration, release=False,
                        pipeline_parallel=None,
                        tensor_rank=None, pipeline_rank=None,
                        expert_parallel=None, expert_rank=None,
                        return_base_dir=False, basename="model_optim_rng.pt", old_pp_num=None, old_ep_num=None):
    ...
    # data parallel rank.
    common_path_lsit = []
    if old_pp_num > mpu.get_pipeline_model_parallel_world_size() and old_ep_num > mpu.get_expert_model_parallel_world_size():
        for pp_id in range(old_pp_num // mpu.get_pipeline_model_parallel_world_size()):
            common_path_pp = os.path.join(checkpoints_path, directory,
                    f'mp_rank_{tensor_rank:02d}_{int(pipeline_rank * (old_pp_num // mpu.get_pipeline_model_parallel_world_size()) + pp_id):03d}')
            
            for ep_id in range(old_ep_num // mpu.get_expert_model_parallel_world_size()):
                common_path = common_path_pp + f'_{int(expert_rank * (old_ep_num // mpu.get_expert_model_parallel_world_size()) + ep_id):03d}'
                
                common_path_lsit.append(os.path.join(common_path, basename))
    elif old_pp_num > mpu.get_pipeline_model_parallel_world_size() and old_ep_num <= mpu.get_expert_model_parallel_world_size():
        for pp_id in range(old_pp_num // mpu.get_pipeline_model_parallel_world_size()):
            common_path_pp = os.path.join(checkpoints_path, directory,
                    f'mp_rank_{tensor_rank:02d}_{int(pipeline_rank * (old_pp_num // mpu.get_pipeline_model_parallel_world_size()) + pp_id):03d}')
            
            common_path = common_path_pp + f'_{int(expert_rank//(mpu.get_expert_model_parallel_world_size()//old_ep_num)):03d}'
        ...
        for ep_id in range(old_ep_num // mpu.get_expert_model_parallel_world_size()):
            ...
            common_path_lsit.append(os.path.join(common_path, basename))
    elif old_pp_num == mpu.get_pipeline_model_parallel_world_size() and old_ep_num == 1:
        common_path_pp = os.path.join(checkpoints_path, directory,
                f'mp_rank_{tensor_rank:02d}_{int(pipeline_rank):03d}')
        common_path_lsit.append(os.path.join(common_path_pp, basename))
    ...
    #         common_path = common_path + f'_{expert_rank:03d}'
    # print("### get_checkpoint_name_with_diff_p pp_rank: {}, ep_rank: {}, path: {}\n".format(mpu.get_pipeline_model_parallel_rank(), mpu.get_expert_model_parallel_rank(), os.path.join(common_path, basename)))
    return common_path_lsit

def _load_base_checkpoint_list_with_diff_p(
    load_dir,
    args,
    rank0=False,
    sharded_state_dict=None,
    ...
    If rank0 is true, just loads rank 0 checkpoint, ignoring arguments.
    """
    # assert (args.old_pp_num is not None and mpu.get_pipeline_model_parallel_world_size() < args.old_pp_num) and (args.old_ep_num is not None and mpu.get_expert_model_parallel_world_size() < args.old_ep_num), "_load_base_checkpoint_list_with_diff_p only support load checkpoint with different parallel type."
    ...
    #     )
    # Handle global legacy checkpoint
    checkpoint_names = get_checkpoint_names_with_diff_p(load_dir, iteration, release, return_base_dir=False, old_pp_num=args.old_pp_num, old_ep_num=args.old_ep_num)
    print("### rank: {}, pp_rank: {}, ep_rank: {}, checkpoints: {}".format(torch.distributed.get_rank(), mpu.get_pipeline_model_parallel_rank(), mpu.get_expert_model_parallel_rank(), checkpoint_names))
    # if rank0:
    #     checkpoint_name = find_checkpoint_rank_0(load_dir, iteration, release)
    # else:
    #     if args.old_pp_num is not None or args.old_ep_num is not None:
    #         checkpoint_name = get_checkpoint_name_with_diff_p(load_dir, iteration, release, return_base_dir=False, old_pp_num=args.old_pp_num, old_ep_num=args.old_ep_num)
    #     else:
    #         checkpoint_name = get_checkpoint_name(load_dir, iteration, release, return_base_dir=False)
    try:
        state_dict_list = []
    ...
    #     sys.modules.pop('megatron.model', None)
        ...
        print(e)
        sys.exit()
    return state_dict_list, checkpoint_names, release, CheckpointType.LEGACY

def _convert_to_new_state_dict(state_dict_list, new_model):
    """
    ...
    """
    args = get_args()
    # mlp = new_model.get_submodule('decoder.layers.0.mlp')
    pp_multiple_num = args.old_pp_num // mpu.get_pipeline_model_parallel_world_size() if args.old_pp_num is not None else 1
    pp_rank = mpu.get_pipeline_model_parallel_rank()
    ...
    # assert pp_multiple_num * ep_multiple_num == len(state_dict_list), "pp, ep not match: {}".format(state_dict_list)

    # print("### ep size: {}, ep rank: {}, ep group: {}, rank: {}\n".format(mpu.get_expert_model_parallel_world_size(), mpu.get_expert_model_parallel_rank(), torch.distributed.get_process_group_ranks(mpu.get_expert_model_parallel_group()), torch.distributed.get_rank()))

    new_model_state_dict = new_model.state_dict().copy()
    for pp_id in range(pp_multiple_num):
        for ep_id in range(ep_multiple_num):
            state_dict = state_dict_list[pp_id + ep_id]["model"]
            # print_rank_0("### new_model.state_dict() len: {}, state_dict len: {}".format(len(new_model.state_dict()), len(state_dict)))
            # print_rank_0(new_model.state_dict().keys())
            # print_rank_0(state_dict.keys())
            # exit(0)
            # for new_key in state_dict:
            #     if "_extra_state" in new_key:
            #         print_rank_0("### key: {} {}".format(new_key, state_dict[new_key]))
            # exit(0)
            # print_rank_0("state_dict[decoder.layers.0.mlp.shared_experts.gate_weight]: {}".format(state_dict['decoder.layers.0.mlp.shared_experts.gate_weight']))
            # exit(0)
            for new_key in new_model.state_dict():
                print_rank_0("### find decoder layers. rank: {}, key: {}".format(torch.distributed.get_rank(), new_key))
                
                if new_key in state_dict: 
                    if "_extra_state" not in new_key:
                        print_rank_0("### new_model_state_dict[{}].shape {}".format(new_key, new_model_state_dict[new_key].shape))
                        print_rank_0("### state_dict[{}].shape {}".format(new_key, state_dict[new_key].shape))
                        print_rank_0("### set new_model_state_dict[{}] == state_dict[{}]".format(new_key, new_key))
                        print_rank_0("")
                        new_model_state_dict[new_key] = state_dict[new_key]
                    else:
                        print_rank_0("### skip: {}".format(state_dict[new_key]))
                        print_rank_0("")
                else:
                    if "mlp.experts.weight1" in new_key:
                        # torch.Size([8, 4096, 16384]))
                        # (8192 * 2) * 8(expert) = 131072
                        # (8192 * 2) * 1(expert) = 16384
                        warnings.filterwarnings(
                            "ignore",
                            message="Cannot create tensor with interal format while allow_internel_format=False"
                        )
                        layername = '.'.join(new_key.split(".")[:-1])
                        layer = new_model.get_submodule(layername)
                        print_rank_0("### hidden_size:{}, gated_linear_unit:{}, num_local_experts:{}, moe_ffn_hidden_size:{}".format(
                            layer.config.hidden_size, layer.config.gated_linear_unit, layer.num_local_experts, layer.config.moe_ffn_hidden_size))
                        pattern = r"(decoder\.layers\.\d+\.mlp\.experts\.)(weight)(\d+)"
                        replacement = r"\1linear_fc1.\2"
                        origin_key = re.sub(pattern, replacement, new_key)
                        print_rank_0("### new_model_state_dict[{}] {}".format(new_key, new_model_state_dict[new_key].shape))
                        for i in range(layer.num_local_experts):
                            idx = i + ep_rank * layer.num_local_experts
                            print_rank_0("### state_dict[{}] {}".format(origin_key+str(idx), state_dict[origin_key+str(idx)].shape))
                            start = i * state_dict[origin_key+str(idx)].shape[0]
                            end = (i+1) * state_dict[origin_key+str(idx)].shape[0]
                            print_rank_0("### set new_model_state_dict[{}][:,{}:{}] = state_dict[{}].transpose(0,1).clone()".format(new_key, start, end, origin_key+str(idx)))
                            new_model_state_dict[new_key][:,start:end] = state_dict[origin_key+str(idx)].transpose(0,1).clone()
                        new_model_state_dict[new_key] = new_model_state_dict[new_key].view([layer.config.hidden_size, layer.num_local_experts, -1])
                        print_rank_0("### new_model_state_dict[{}] {}".format(new_key, new_model_state_dict[new_key].shape))
                        new_model_state_dict[new_key] = new_model_state_dict[new_key].transpose(0,1)
                        print_rank_0("### new_model_state_dict[{}] {}".format(new_key, new_model_state_dict[new_key].shape))
                        # print_rank_0(new_model_state_dict[new_key][0])
                        # print_rank_0(state_dict["decoder.layers.0.mlp.experts.linear_fc1.weight0"])
                        new_model_state_dict[new_key] = new_model_state_dict[new_key].view((layer.config.hidden_size,-1))
                        print_rank_0("### new_model_state_dict[{}] {}".format(new_key, new_model_state_dict[new_key].shape))
                        print_rank_0("")
                    elif "mlp.experts.weight2" in new_key:
                        # (8192 * 2) * 8(expert) = 131072
                        # (8192 * 2) * 1(expert) = 16384
                        warnings.filterwarnings(
                            "ignore",
                        ...
                        print_rank_0("### hidden_size:{}, gated_linear_unit:{}, num_local_experts:{}, moe_ffn_hidden_size:{}".format(
                            layer.config.hidden_size, layer.config.gated_linear_unit, layer.num_local_experts, layer.config.moe_ffn_hidden_size))
                        pattern = r"(decoder\.layers\.\d+\.mlp\.experts\.)(weight)(\d+)"
                        replacement = r"\1linear_fc2.\2"
                        origin_key = re.sub(pattern, replacement, new_key)
                        print_rank_0("### new_model_state_dict[{}] {}".format(new_key, new_model_state_dict[new_key].shape))
                        for i in range(layer.num_local_experts):
                            idx = i + ep_rank * layer.num_local_experts
                            print_rank_0("### state_dict[{}] {}".format(origin_key+str(idx), state_dict[origin_key+str(idx)].shape))
                            start = i * state_dict[origin_key+str(idx)].shape[1]
                            end = (i+1) * state_dict[origin_key+str(idx)].shape[1]
                            print_rank_0("### set new_model_state_dict[{}][{}:{},] = state_dict[{}].transpose(0,1).clone()".format(new_key, start, end, origin_key+str(idx)))
                            new_model_state_dict[new_key][start:end,:] = state_dict[origin_key+str(idx)].transpose(0,1).clone()
                        print_rank_0("")
                    else:
                        print_rank_0("### new key not in state_dict: {}\n".format(new_key))
                        print_rank_0("")
                # else:
                #     # print("### no layers rank: {}, key: {}\n".format(torch.distributed.get_rank(), key))
                #     print_rank_0("### find no-decoder layer. rank: {}, key: {}".format(torch.distributed.get_rank(), new_key))
                #     if new_key in state_dict:
                #         try:
                #             print_rank_0("### new_model_state_dict[{}].shape {}".format(new_key, new_model_state_dict[new_key].shape))
                #             print_rank_0("### state_dict[{}].shape {}".format(new_key, state_dict[new_key].shape))
                ...
                #             print_rank_0("")
                #         except:
                #             print_rank_0("!!! {} error".format(new_key))
                #             exit(0)
                #         new_model_state_dict[new_key] = state_dict[new_key]
                #     else:
                #         if (pp_rank == 0 and pp_id == 0) or (pp_rank == mpu.get_pipeline_model_parallel_world_size()-1 and pp_id == pp_multiple_num-1):
                #             # or (pp_rank == 7 and pp_id == 1):
                #             print_rank_0("### first stage, last stage new key not in state_dict: {}\n".format(new_key))
                #         else:
                #             print_rank_0("### can be ignored new key not in state_dict: {}\n".format(new_key))
    # exit(0)
...
def load_checkpoint(ddp_model, optimizer, opt_param_scheduler, load_arg='load', strict=True,
    ...
    load_kwargs = {}
    if (args.old_pp_num is not None and args.old_pp_num == mpu.get_pipeline_model_parallel_world_size()) and \
    (args.old_ep_num is not None and args.old_ep_num == 1):
        state_dict_list, checkpoint_names, release, ckpt_type = _load_base_checkpoint_list_with_diff_p(
    ...
    if not skip_load_to_model_and_opt:
        if len(ddp_model) == 1:
            ...
            (args.old_ep_num is not None and args.old_ep_num == 1):
                new_state_dict = {}
                new_state_dict['model'] = _convert_to_new_state_dict(state_dict_list, ddp_model[0])
                ddp_model[0].load_state_dict(new_state_dict['model'], strict=strict)
            else:
