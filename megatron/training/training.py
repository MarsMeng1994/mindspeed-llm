...
from megatron.inference.text_generation.tokenization import (
    tokenize_prompts_local,
    detokenize_generations,
    local_tokenizer)
import transformers
from megatron.core.pipeline_parallel import get_forward_backward_func, forward_backward_pipelining_without_interleaving_wo_data
from megatron.inference.text_generation.sampling import (
    sample)
...
def pretrain(
    ...
    if args.do_chat:
        print("### doing  chat\n")
        chat(forward_step_func, model=model, config=config)
...
def chat(forward_step_func,
    ...
    # eval_num_microbatches = eval_batch_size // \
    #     (args.micro_batch_size * args.data_parallel_size)
    # 全局预分配（假设 batch=1，vocab=50257）
    MASK = torch.empty(1, 151680, dtype=torch.bool, device='cuda')

    def apply_repetition_penalty(logits, pen_ids, penalty=1.5):
        MASK.zero_()                           # 全部置 False
        for t in pen_ids:
            MASK[0, t] = True                  # 标记出现过的 token
    ...
    print("### doing caht with length {} ===============================================\n".format(window_size))

    should_stop = torch.tensor([0], dtype=torch.long, device=torch.cuda.current_device())

    if mpu.is_pipeline_first_stage() and mpu.get_tensor_model_parallel_rank() == 0:
            #and torch.distributed.get_rank()==0
        user_input = "我去早市买东西，买了一斤苹果，2.5千克香蕉，苹果是3元每斤，香蕉是2元每斤，我一共要花多少钱？注意单位换算"
        
        # print("### input tokens: {}\n".format(context_tokens_tensor))
        user_input_format = "<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n{}<|im_end|>\n<|im_start|>assistant\n".format(user_input)
        context_tokens_tensor, context_length_tensor = tokenize_prompts_local(
                    prompts=[user_input_format], tokens_to_generate=window_size, add_BOS=False,
                    data_parallel=False, rank=torch.distributed.get_rank()
                    )
    ...
    with torch.no_grad():
        ...
        if verbose:
            print_rank_0(f'Evaluating on {args.eval_iters * eval_batch_size} samples')
        while iteration < window_size:
            if mpu.is_pipeline_first_stage() and mpu.get_tensor_model_parallel_rank() == 0:
                if iteration > 0 :
                    print("### rank: {}, iteration: {}, new_sample detokenized: {}\n".format(torch.distributed.get_rank(), iteration, local_tokenizer.batch_decode([new_sample], clean_up_tokenization_spaces=False, spaces_between_special_tokens=False)[0]))
                    # user_input_format = user_input_format + local_tokenizer.batch_decode([new_sample], clean_up_tokenization_spaces=False, spaces_between_special_tokens=False)[0]
                    context_tokens_tensor[0, context_length] = new_sample
                    context_length = context_length + 1
                    # print("### rank: {}, iteration: {}, user_input: {}\n".format(torch.distributed.get_rank(),iteration, user_input_format))
                    print("### rank: {}, iteration: {}, user_input: {}\n".format(torch.distributed.get_rank(),iteration, local_tokenizer.batch_decode(context_tokens_tensor[:, :context_length], clean_up_tokenization_spaces=False, spaces_between_special_tokens=False)[0]))
                    
                    
                # user_input_format = "<|im_start|>system\nYou are a helpful assistant.<|im_end|>\n<|im_start|>user\n{}<|im_end|>\n<|im_start|>assistant\n".format(user_input)

                
                # print("### rank: {}, iteration: {}, input tokens: {}, context_length: {}\n".format(torch.distributed.get_rank(), iteration, context_tokens_tensor, context_length))
                print("### rank: {}, iteration: {}, context_length: {}\n".format(torch.distributed.get_rank(), iteration, context_length))
            
            
            # if verbose:
            #     print_rank_0(f'Evaluating iter {iteration}/{args.eval_iters}')

            # print("### rank: {}, iteration: {}, before barrier\n".format(torch.distributed.get_rank(), iteration))
            torch.distributed.barrier(group=mpu.get_pipeline_model_parallel_group())
            # print("### rank: {}, iteration: {}, after barrier\n".format(torch.distributed.get_rank(), iteration))

            # forward_func = forward_backward_pipelining_without_interleaving_wo_data()
            # Don't care about timing during evaluation
            # config.timers = None
            # ft_integration.on_eval_step_start()
            if mpu.is_pipeline_last_stage(ignore_virtual=True):
                forward_data_store = forward_backward_pipelining_without_interleaving_wo_data(
                    forward_step_func=forward_step_func,
                    # data_iterator: Union[Iterator, List[Iterator]],
                    model=model,
                    num_microbatches=1,
                    seq_length=window_size,
                    input_tensor=context_tokens_tensor,
                    micro_batch_size=1,
                    decoder_seq_length=args.decoder_seq_length,
                    forward_only=True,
                    )
            else:
                _ = forward_backward_pipelining_without_interleaving_wo_data(
                    forward_step_func=forward_step_func,
                    # data_iterator: Union[Iterator, List[Iterator]],
                    model=model,
                    num_microbatches=1,
            ...
            if mpu.is_pipeline_last_stage(ignore_virtual=True):
                # print("### rank: {}, forward_data_store: {}, dtype; {}\n".format(torch.distributed.get_rank(), forward_data_store, forward_data_store[0].dtype))
                # output_tensor = 
                logits = forward_data_store[0].contiguous().to(torch.float32)
                # print("### rank: {}, logits:{}, logits shape: {}, dtype:{}\n".format(torch.distributed.get_rank(), logits, logits.shape, logits.dtype))

                # last_token_logits = logits[:, context_length_tensor, :]
                # new_sample = sample(last_token_logits,
                #                     top_k=1,
                #                     top_p=0.,
                #                     temperature=0.95,
                #                     vocab_size=len(local_tokenizer))
                # print("### rank: {}, new_sample: {}\n".format(torch.distributed.get_rank(), new_sample))
                copy_from_last_to_first_pipeline_stage([1, window_size, 151680], torch.float32,
                                            logits)
            elif mpu.is_pipeline_first_stage(ignore_virtual=True):
                logits = torch.empty(
                            [1, window_size, 151680],
                            device=torch.cuda.current_device(),
                            dtype=torch.float32,
                        ).contiguous()
                # print("### rank: {}, logits before: {}, shape: {}\n".format(torch.distributed.get_rank(), logits, logits.shape))

                copy_from_last_to_first_pipeline_stage([1, window_size, 151680], torch.float32,
                                            logits)
                                
                # print("### rank: {}, logits after: {}, shape: {}\n".format(torch.distributed.get_rank(), logits, logits.shape))
            else:
                pass
            
            if mpu.is_pipeline_first_stage(ignore_virtual=True):
                # print("### rank0: {}, new_sample: {}, shape: {}\n".format(torch.distributed.get_rank(), logits, logits.shape))
                
                last_token_logits = logits[:, context_length-1, :].squeeze(1).float()
                # last_token_logits = apply_repetition_penalty(last_token_logits, [29333, 34369])
                new_sample = sample(last_token_logits,
                                    top_k=args.do_chat_topk,
                                    top_p=args.do_chat_topp,
                                    temperature=args.do_chat_temperature,
                                    vocab_size=151680)
                print("### rank: {}, new_sample token: {}\n".format(torch.distributed.get_rank(), new_sample))
                if int(new_sample.item()) == 151645:
                    print("### rank: {}, stop\n".format(torch.distributed.get_rank()))
                    print("[total output]: {}\n".format(local_tokenizer.batch_decode(context_tokens_tensor[:, :context_length], clean_up_tokenization_spaces=False, spaces_between_special_tokens=False)[0]))
                    should_stop.fill_(1)
            ...
            if mpu.is_pipeline_last_stage(ignore_virtual=True):
                ...
                # del context_tokens_tensor
                # del context_length_tensor
            else:
                pass
            # print("### rank: {}, iteration: {} complete\n".format(torch.distributed.get_rank(), iteration))
                # print("### rank0: {}, detokenize new_sample: {}\n".format(torch.distributed.get_rank(), local_tokenizer.batch_decode([logits])))

            if torch.distributed.get_rank() % 2 == 0:
                torch.distributed.broadcast(should_stop, src=0, group=mpu.get_pipeline_model_parallel_group())
            else:
                torch.distributed.broadcast(should_stop, src=1, group=mpu.get_pipeline_model_parallel_group())
