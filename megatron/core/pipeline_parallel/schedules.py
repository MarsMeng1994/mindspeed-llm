...
def forward_step_wo_loss(
    ...
    unwrap_output_tensor = False
    if not isinstance(input_tensor, list):
        input_tensor_lst = [input_tensor]
        # unwrap_output_tensor = True

    set_input_tensor = get_attr_wrapped_model(model, "set_input_tensor")
    set_input_tensor(input_tensor_lst)
    ...
    if config.enable_autocast:
        ...
        context_manager = contextlib.nullcontext()
    with context_manager:
        output_tensor = forward_step_func(input_tensor, position_ids, model)
...
def forward_backward_pipelining_without_interleaving_wo_data(
    *,
    forward_step_func,
    # data_iterator: Union[Iterator, List[Iterator]],
    model: Union[torch.nn.Module, List[torch.nn.Module]],
    ...
    if rank != 0:
        ...
        # position_ids = torch.arange(0, len(input_tensor))
        position_ids = None

    else:
        assert input_tensor != None
        
        position_ids = torch.arange(0, len(input_tensor[0]))

    # print("### rank: {}, input tensor: {}, shape: {}, position_ids: {}\n".format(torch.distributed.get_rank(), input_tensor, input_tensor.shape, position_ids))
    output_tensor, num_tokens = forward_step_wo_loss(
        forward_step_func,
        model,
        num_microbatches,
        input_tensor,
    ...
    #         checkpoint_activations_microbatch = None
        ...
        #     checkpoint_activations_microbatch,
        #     check_first_val_step(
        #         first_val_step, forward_only, (i == 0) and (0 == 0)
        #     ),
        #     current_microbatch=0
        #     encoder_decoder_xattn=encoder_decoder_xattn,
        # )
