...
from megatron.core.ssm.mamba_hybrid_layer_allocation import allocate_layers, allocate_layers_moe
...
@dataclass
class MambaMoEStackSubmodulesV3:
    mamba_moe_layer: Union[ModuleSpec, type] = IdentityOp
    transformer_layer: Union[ModuleSpec, type] = IdentityOp

class MambaMoEStackV3(MegatronModule):
    def __init__(
        self,
        config: TransformerConfig,
        ...
        self.hybrid_attention_ratio = hybrid_attention_ratio
        self.hybrid_attention_freq = int(1 / self.hybrid_attention_ratio)
        ...
        for idx in range(self.config.num_layers):
            layer_id = idx+1
            if layer_id % self.hybrid_attention_freq == 0:
                layer_type_list.append("*#")
            else:
                layer_type_list.append("M#")
        # layer_type_list = allocate_layers_moe(
        #     self.config.num_layers,
        #     self.hybrid_attention_ratio,
        #     self.hybrid_mlp_ratio,
        #     self.hybrid_override_pattern,
        #     self.config.hybrid_mlp_moe,
        # )
        # ['M', '-', 'M', '-', 'M', '-', '-', 'M', '-', '*', 'M', '-', 'M', '-', 'M', '-', 'M', '-', '-', '*', 'M', '-', 'M', '-', 'M', '-', 'M', '-', '*', 'M', '-', '-', 'M', '-', 'M', '-', 'M', '-', '*', 'M', '-', 'M', '-', '-', 'M', '-', 'M', '-']
        print_rank_0(f"########## layer_type_list: {layer_type_list}")
        print_rank_0(f"########## self.config: {self.config}")
        ...
        print("### pp rank: {}, pp_layer_offset:{}, layer_type_list:{}\n".format(parallel_state.get_pipeline_model_parallel_rank(), pp_layer_offset, layer_type_list))
        self.layers = nn.ModuleList()
        self.layer_type_list = []
        for i, layer_type in enumerate(layer_type_list):

            if layer_type == LayerSymbols.MAMBA_MOE:
                layer = build_module(
                    submodules.mamba_moe_layer,
                    config=self.config,
                    mamba_ssm_ngroups=mamba_ssm_ngroups,
                    layer_number=i + 1 + pp_layer_offset,
                    residual_in_fp32=residual_in_fp32,
                )
            elif layer_type == LayerSymbols.TRANSFORMER_MOE:
                # Transformer layers apply their own pp_layer_offset
                # if (i + 1) % self.config.input_conv_freq == 1 and (i + 1) % self.config.knowledge_block_freq == 1:
                #     layer = build_module(
                #         submodules.attention_moe_layer, config=self.config, layer_number=i + 1
                #     )
                #     skip_next = True
                # else:
                layer = build_module(
                    submodules.transformer_layer, config=self.config, layer_number=i + 1,
                )
            # elif layer_type == LayerSymbols.MLP:
            #     print("submodules.mlp_layer", submodules.mlp_layer)
            #     # Transformer layers apply their own pp_layer_offset
            #     layer = build_module(submodules.mlp_layer, config=self.config, layer_number=i + 1)
            # elif layer_type == LayerSymbols.MOE:
            #     # Transformer layers apply their own pp_layer_offset
            #     print("submodules.moe_layer", submodules.moe_layer)
        ...
        self.apply(partial(_init_weights, n_layer=self.config.num_layers, n_residuals_per_layer=2))

    def _select_layers_for_pipeline_parallel(self, layer_type_list):
        pipeline_rank = parallel_state.get_pipeline_model_parallel_rank()
        pipeline_rank_total = parallel_state.get_pipeline_model_parallel_world_size()
        num_layers = self.config.num_layers
        num_layers_first_satge = 0
        num_layers_last_stage = 0
        if self.config.num_layers_in_first_pipeline_stage:
            num_layers = num_layers - self.config.num_layers_in_first_pipeline_stage
            pipeline_rank_total = pipeline_rank_total - 1
            # pipeline_rank = pipeline_rank -1
            num_layers_first_satge = self.config.num_layers_in_first_pipeline_stage
        if self.config.num_layers_in_last_pipeline_stage:
            num_layers = num_layers - self.config.num_layers_in_last_pipeline_stage
            pipeline_rank_total = pipeline_rank_total - 1

            num_layers_last_stage = self.config.num_layers_in_last_pipeline_stage

        num_layers_loal_rank = 0
        if parallel_state.is_pipeline_last_stage() and num_layers_last_stage != 0:
            num_layers_loal_rank = num_layers_last_stage
        elif parallel_state.is_pipeline_first_stage() and num_layers_first_satge != 0:
            num_layers_loal_rank = num_layers_first_satge
        # else:
        num_layers_per_pipeline_rank = (
            num_layers // pipeline_rank_total
        )
        ...
        if parallel_state.is_pipeline_first_stage():
            offset = 0
            selected_list = layer_type_list[offset : offset + num_layers_loal_rank if num_layers_loal_rank != 0 else offset + num_layers_per_pipeline_rank]
        elif not parallel_state.is_pipeline_last_stage():
            if num_layers_first_satge != 0:
                offset = num_layers_per_pipeline_rank * (pipeline_rank - 1) + num_layers_first_satge
            else:
                offset = num_layers_per_pipeline_rank * pipeline_rank
            selected_list = layer_type_list[offset : offset + num_layers_per_pipeline_rank]
        else:
            if num_layers_first_satge != 0:
                offset = num_layers_per_pipeline_rank * (pipeline_rank - 1) + num_layers_first_satge
        ...
        # if pipeline_rank < 0:
        #     offset = 0
        #     selected_list = layer_type_list[offset : offset + num_layers_loal_rank]
        # else:
        #     offset = pipeline_rank * num_layers_per_pipeline_rank
        #     if num_layers_first_satge != 0:
        #         offset = offset + num_layers_first_satge
        #     selected_list = layer_type_list[offset : offset + num_layers_per_pipeline_rank if num_layers_loal_rank != 0 else offset + num_layers_loal_rank]
    ...
    def _checkpointed_forward(
        ...
        def checkpoint_handler(forward_func):
            ...
            if self.config.fp8:
                # print("### checkpoint_handler self.config.fp8: {}\n".format(self.config.fp8))
    ...
    def forward(
        ...
        with rng_context, outer_fp8_context:
            ...
            if self.config.recompute_granularity == 'full' and self.training:
                ...
                for layer, symbol, idx in zip(self.layers, self.layer_type_list, range(len(self.layer_type_list))):
                    hidden_states, context = layer(
                        hidden_states,
                        attention_mask,
                    ...
                    # print("### MambaMoEStackV3 forward rank: {}, layer_idx: {}, hidden_states: {}\n".format(torch.distributed.get_rank(), idx, hidden_states))
                if isinstance(hidden_states, tuple):
                    hidden_states = hidden_states[0]
