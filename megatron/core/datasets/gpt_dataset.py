...
class GPTDatasetConfig(BlendedMegatronDatasetConfig):
    ...
    is_sft_dataset: bool = None
    """Whether to use sft gpt dataset."""
...
class GPTDataset(MegatronDataset):
    ...
    def __init__(
        ...
        # print("### gptdataset init indexed_indices: {}\n".format(indexed_indices))
        super().__init__(
        ...
        # print("### self.document_index: {}\n self.sample_index: {}\n self.shuffle_index:{}\n".format(self.document_index, self.sample_index, self.shuffle_index))
    @staticmethod
    ...
    def build_low_level_dataset(dataset_path: str, config: GPTDatasetConfig) -> IndexedDataset:
        ...
        """
        # print("### build_low_level_dataset is_s3_path {}: {}\n".format(dataset_path, is_s3_path(dataset_path)))
    ...
    def __getitem__(self, idx: Optional[int]) -> Dict[str, torch.Tensor]:
        ...
        # if len(text) != 4097:
        # print("first read txt: {}\n".format(text))
        if self.config.is_sft_dataset:
            assert len(text) % 2 == 0, f'tokens length must be divisible by 2, len(text): {len(text)}'
            # text = torch.from_numpy(text).long()
            half_len = int(len(text) / 2)
            tokens, labels = text[:half_len], text[half_len:]
            ...
            if len(tokens) > tmp:
                ...
                num_pads = self.config.sequence_length + int(self.config.add_extra_token_to_sequence) - len(tokens)
                tokens = torch.nn.functional.pad(tokens, pad=(0, num_pads), mode='constant', value=self.config.tokenizer.eod)
                labels = torch.nn.functional.pad(labels, pad=(0, num_pads), mode='constant', value=-100)
                # attention_mask = torch.nn.functional.pad(attention_mask, pad=(0, num_pads), mode='constant', value=0)

            tokens = tokens[:-1].contiguous()
            labels = labels[1:].contiguous()
            
            # attention_mask = attention_mask[:-1].contiguous() < 0.5
    ...
    def _query_document_sample_shuffle_indices(
        ...
        # print("### _query_document_sample_shuffle_indices doc_index_beg: {}\n doc_index_beg_offset: {}\n".format(doc_index_beg, doc_index_beg_offset))
        # print("### _query_document_sample_shuffle_indices doc_index_end: {}\n doc_index_end_offset: {}\n".format(doc_index_end, doc_index_end_offset))

        document_ids = []
        sample_parts = []

        if self.config.is_sft_dataset:
            assert doc_index_end - doc_index_beg == 1, f"doc_index_end: {len(doc_index_end)} != doc_index_beg: {len(doc_index_beg)} + 1"
            document_ids.append(self.document_index[doc_index_beg])
            token_ids = self.dataset.get(
                    self.document_index[doc_index_beg],
                    offset=doc_index_beg_offset,
                    length=None,
                    # + self.config.add_extra_token_to_sequence,
                )
            sample_parts.append(token_ids)
        else:
            ...
            if doc_index_beg == doc_index_end:
                ...
                # print("### _query_document_sample_shuffle_indices txt: {} \n len:{} Add the entire sample: {}\n".format(local_tokenizer.decode(token_ids=token_ids), len(token_ids), token_ids))
            ...
            # Sample spans multiple documents
                ...
                for i in range(doc_index_beg, doc_index_end + 1):
                    ...
                    token_ids = self.dataset.get(self.document_index[i], offset=offset, length=length)
                    # token_ids_orig_len_list.append(len(token_ids))
                    if i < doc_index_end:
                        if token_ids[-1] != 151645 or token_ids[-1] != 151643:
                            # token_ids.append(151645)
                            token_ids = numpy.append(token_ids, 151645)
                            add_idx_count = add_idx_count + 1
                    else:
                        if add_idx_count > 0:
                            token_ids = token_ids[:-add_idx_count]
                    total_len = total_len + len(token_ids)
                    if total_len > (self.config.sequence_length + self.config.add_extra_token_to_sequence):
                        token_ids = token_ids[:self.config.sequence_length + self.config.add_extra_token_to_sequence - total_len]
                        sample_parts.append(
                            token_ids
                        )
                        break
                    elif total_len == (self.config.sequence_length + self.config.add_extra_token_to_sequence):
                        sample_parts.append(
                            token_ids
                        )
                        break
                    # add_idx_count_list.append(add_idx_count)
                    # token_ids_len_list.append(len(token_ids))
                    
                    # print("### _query_document_sample_shuffle_indices len: {} \n true len: {}\n Add the sample part: {}\n txt: {}\n".format(length, len(token_ids), token_ids, local_tokenizer.decode(token_ids=token_ids) ))
                    # print("### _query_document_sample_shuffle_indices txt: {} \n len:{} Add the sample: {}\n".format(local_tokenizer.decode(token_ids=token_ids), len(token_ids), token_ids))
            ...
            # print("### length < (self.config.sequence_length + self.config.add_extra_token_to_sequence): {}\n".format(length < (self.config.sequence_length + self.config.add_extra_token_to_sequence)))
            # print("### total sample_parts len: {}\n self.config.sequence_length: {}\n self.config.add_extra_token_to_sequence: {}\n".format(length, self.config.sequence_length, self.config.add_extra_token_to_sequence))

            # assert length == 4097, "wrong length!!!!!!!!!!!!!! offset_len_list: {}\nlength_list: {}\ntoken_ids_orig_len_list: {}\ntoken_ids_len_list: {}\nadd_idx_count_list: {}\nif: {}, len:{}, ids:{}\n".format(offset_len_list, length_list, token_ids_orig_len_list, token_ids_len_list, add_idx_count_list, length < (self.config.sequence_length + self.config.add_extra_token_to_sequence), length, sample_parts)
        ...
        # assert length == 4097, "wrong length!!!!!!!!!!!!!! after add"
    ...
    def _build_document_sample_shuffle_indices(
        ...
        # print("### _build_document_sample_shuffle_indices path_to_cache: {}\n".format(path_to_cache))
        ...
        if not path_to_cache or (
            ...
            # print("### _build_document_sample_shuffle_indices num_tokens_per_epoch: {}\n".format(num_tokens_per_epoch))
            # print("### _build_document_sample_shuffle_indices self.dataset.sequence_lengths: {}\n".format(self.dataset.sequence_lengths))

            # print("### _build_document_sample_shuffle_indices num_tokens_per_epoch: {}\n".format(num_tokens_per_epoch))

            num_epochs = self._get_num_epochs(num_tokens_per_epoch)
            # print("### _build_document_sample_shuffle_indices num_epochs: {}\n".format(num_epochs))
            if num_epochs == 1:
            ...
            # print("### _build_document_sample_shuffle_indices self.indices: {}, num_epochs: {}, separate_final_epoch: {}\n".format(self.indices, num_epochs, separate_final_epoch))

            document_index = _build_document_index(
                self.indices, num_epochs, numpy_random_state, separate_final_epoch
            )
            # print("### _build_document_sample_shuffle_indices document_index: {}\n".format(document_index))
            ...
            # print("### _build_document_sample_shuffle_indices docusample_indexment_index: {}\n".format(sample_index))
...
def build_sft_sample_idx(num_samples):
    """
    Sample index (sample_idx) is used for GPT-2 like dataset for which the documents are flattened
    ...
    dtype = numpy.int32
    sample_idx = numpy.zeros((num_samples + 1, 2), dtype=dtype)
    sample_idx[:, 0] = numpy.arange(len(sample_idx)) 
    return sample_idx
