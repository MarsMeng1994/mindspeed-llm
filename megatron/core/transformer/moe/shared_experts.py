...
class SharedExpertMLP(MLP):
    ...
    def __init__(self, config: TransformerConfig, submodules: MLPSubmodules, gate: bool):
        ...
        self.use_shared_expert_gate = True
        print("### SharedExpertMLP use_shared_expert_gate: {}\n".format(self.use_shared_expert_gate))
    ...
    def get_output(self, shared_probs: Optional[torch.Tensor] = None):
        """
        ...
        with torch.cuda.stream(self.stream):
            if self.use_shared_expert_gate:
                ...
                if shared_probs is not None:
                    output = self.cached_output * shared_probs.view(self.cached_output.size(0), self.cached_output.size(1), -1)
                else:
